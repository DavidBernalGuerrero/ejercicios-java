...
public class Disco {
  ...
  public Disco (ArrayList<Disco> discos) {
  ...
  public static void añadeDisco (ArrayList<Disco> discos) {
    
    boolean anadido = false; //Una vez la operación se realiza con éxito sale del bucle.
    
    for (int i = 0; (i < discos.size()) && (!anadido); i++) {
      if (discos.get(i) == null) {
        discos.add(new Disco(discos));
        discos.get(i).setEstado("OCUPADO");
        anadido =  true;
      }
    }
  ...
  public static void modificaDisco (ArrayList<Disco> discos) {
    ...
    for (int i = 0; (i < discos.size()) && (!modificado); i++) {
      if ((discos.get(i) != null) && (discos.get(i).getCodigo().equals(referencia))) {
        while (!salir) {
  ...
  public static void borraDisco (ArrayList<Disco> discos) {
    System.out.println("Introduce el código del disco que desea borrar: ");
    String referencia = s.nextLine();
    boolean borrado = false;
    
    for (int i = 0; (i < discos.size()) && (!borrado); i++) {
      if ((discos.get(i) != null) && (discos.get(i).getCodigo().equals(referencia))) {
        discos.set(i, null);
        borrado = true;
  ...
  public static void muestraLista (ArrayList<Disco> discos) {
  ...
  private static boolean compruebaExistencia(ArrayList<Disco> discos, String codigoReferencia) {
    
    for (int i = 0; i < discos.size(); i++){
      if ((discos.get(i) != null) && (discos.get(i).getCodigo().equals(codigoReferencia))) {
        return true;
      }
    }
